...
I am currently a fourth year Ph.D student at the University of Wisconsin -
Madison studying computer science. I'm working on choreographic programming as
part of the [madPL](https://madpl.cs.wisc.edu/) group with my advisor
[Ethan Cecchetti](https://cecchetti.sites.cs.wisc.edu/).

Previously, I was fortunate to be advised by [Tej
Chajed](https://www.chajed.io/) before he moved to Google DeepMind. While
working with Tej, my work focused on software verification, roughly speaking how
we can formally prove that programs behave correctly under all
circumstances. 
...
semesters, and a Lab Instructor for one semester. 

During the summer of 2021, I participated in the Google Summer of Code by
working with [NetworkX](https://networkx.org/) on implementing an Asymmetric
...
polygons. During the summer of 2024, I overhauled the graph visualization API in
NetworkX, moving to an API design were all information is read off the graph
directly.
...
My research interests lay in programming languages and concurrent
systems. Choreographic programming is a recent paradigm for programming
concurrent systems where rather than providing one program for each member of
the system, the developer writes one large program which is then compiled into a
set of programs, one for each member of the system, automatically. Choreographic
programs are provably deadlock-free by construction, eliminating whole classes
of hard to correct bugs statically. While current choreographic languages
provide excellent guarantees, they use assumptions about the execution
environment which simply don't hold in the real world, like that processes can't
fail and the network is perfectly synchronous and won't drop messages. I'm
interested in exploring how to relax these assumptions so that we can build
practical systems using choreographic languages and deploy them in the real
world without losing the strong theoretical guarantees of current choreographic
languages.

Previous projects of mine also focus on distributed systems and verification,
but not choreographic programming. One such project, started under the direction
of Tej Chajed, seeks to reason about software updates, ensuring that a new
...
to be compatible with another. Before that I worked on
...
My venture into the field of programming languages has been extremely rewarding,
blending all of the aspects I love about theory with practical applications
about how we can reason about the behavior of programs.

## Other Interests

Computer science education is a secondary topic of interest for me. While
working with students over the last 5 years, I've seen a lot of different
...
Stars Without Number, watching movies, exploring the outdoors and cooking.
...
Instructing CS 400 during 2026 summer term at the University of Wisconsin -
Madison.
...
Published <a href="https://ieeexplore.ieee.org/document/11401899">ShapeToVec:
Encoding Polygonal Shapes with Extreme Area Variability for Effective
Approximate Jaccard Similarity Queries</a>.
...
Rewrote the existing visualization API to be agnostic to the visualization tool
(in this case <code>matplotlib</code>), provide more flexibility and the
potential for visualization dispatching in the future. {{</timelineItem>}}
...
Pursuing a Ph.D. in Computer Science. {{< /timelineItem >}}
...
Worked on finding the approximate nearest neighbors of polygons using locality
sensitive hashing. The work resulted in a multi-threaded C++ system using the
developed techniques. {{< /timelineItem >}}
